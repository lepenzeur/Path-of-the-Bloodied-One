...
def v92_blacksmith_upgrade_apply(kind, price):
    ...
    if int(v92_blacksmith_upgrades.get("weapon", 0)) >= 1:
        blacksmith_mesaji = bt(
            "Bu dövümü bir kez yaptım. Aynı çeliği ikinci kez ikiye katlayamam.",
            "I have already done this forging once. I cannot double the same steel twice.",
        )
        return False
    ...
    merchant_islem_sesi_cal("buy")
    blacksmith_mesaji = bt(
        "Dövüm tamam. Gücün iki katına çıktı.",
        "The forging is complete. Your power has doubled.",
    )
    dunya_olayi_kaydet("blacksmith_power_double", level=int(oyuncu_level), price=int(price))
    return True


def v92_blacksmith_haggle_begin(kind):
    global blacksmith_mesaji
    if str(kind) == "weapon" and int(v92_blacksmith_upgrades.get("weapon", 0)) >= 1:
        blacksmith_mesaji = bt(
            "Güç dövümü zaten tamamlandı.",
            "The power forging is already complete.",
        )
        return False
    return _v106_blacksmith_haggle_previous(kind)
...
def _v95_blacksmith_rows():
    ...
    if blacksmith_sayfa == "upgrade":
        for row in rows:
            if row.get("id") != "weapon":
                continue
            done = int(v92_blacksmith_upgrades.get("weapon", 0)) >= 1
            row["name"] = bt(
                "GÜÇ x2 · TAMAMLANDI" if done else "GÜCÜ İKİYE KATLA",
                "POWER x2 · COMPLETE" if done else "DOUBLE POWER",
            )
            row["description"] = bt(
                "Reinald mevcut kuvvet ve gerçek saldırı gücünü bir kez ikiye katlar. "
                "Bedel karakter seviyesi yükseldikçe artar.",
                "Reinald doubles current strength and actual attack power once. "
                "The price rises with character level.",
            )
