...
def _adefo_normal_saldiriyi_commit_et(simdi):
    ...
    base_attack_ms = PRECIOSA_NORMAL_SURE_MS if karakter_cinsiyet == "female" else ADEFO_NORMAL_SURE_MS
    oyuncu_saldiri_sure_ms = max(1, int(round(float(base_attack_ms) / max(1.0, v116_weapon_attack_speed_multiplier()))))
...
GELISTIRICI_TEST_TUSLARI.update({
    ...
    pygame.K_u, pygame.K_i, pygame.K_o, pygame.K_p, pygame.K_QUOTE,
})
...
def _agraphon_test_alchemy_grant():
    aurum_ok = envantere_item_ekle(aurum_potabile_olustur(), kazanimi_goster=False)
    essentia_ok = envantere_item_ekle(quinta_essentia_olustur(), kazanimi_goster=False)
    if aurum_ok and essentia_ok:
        bildirim_goster("Aurum Potabile +1 · Quinta Essentia +1", V91_UI_GOLD)
    elif aurum_ok:
        bildirim_goster(bt("Aurum Potabile +1 · Essentia için envanter dolu.", "Aurum Potabile +1 · Inventory full for Essentia."), V91_UI_RED_HOT)
    elif essentia_ok:
        bildirim_goster(bt("Quinta Essentia +1 · Aurum için envanter dolu.", "Quinta Essentia +1 · Inventory full for Aurum."), V91_UI_RED_HOT)
    else:
        bildirim_goster(bt("Envanter dolu.", "Inventory is full."), V91_UI_RED_HOT)
    return True


AGRAPHON_DEV_ALL_SKILLS = False
_agraphon_v45_skill_unlocked_raw = v45_skill_unlocked


def v45_skill_unlocked(skill_id):
    if GELISTIRICI_MODU and AGRAPHON_DEV_ALL_SKILLS:
        return True
    return _agraphon_v45_skill_unlocked_raw(skill_id)


def _agraphon_test_unlock_all_skills():
    global gelistirici_x_skill_aktif, AGRAPHON_DEV_ALL_SKILLS
    AGRAPHON_DEV_ALL_SKILLS = True
    v92_training["decussatio_rubra"] = 5
    v92_training["catena_decollationis"] = 5
    gelistirici_x_skill_aktif = True
    bildirim_goster(bt("Bütün yetenekler test için açıldı.", "All abilities unlocked for testing."), V91_UI_GOLD)
    return True


_agraphon_injury_update_raw = v90_injury_update

def v90_injury_update(now, dt):
    global saldiri_bekleme_suresi
    result = _agraphon_injury_update_raw(now, dt)
    saldiri_bekleme_suresi = max(1, int(round(float(saldiri_bekleme_suresi) / max(1.0, v116_weapon_attack_speed_multiplier()))))
    return result
...
def gelistirici_test_girdisi_uygula(olay):
    ...
    if olay.key == pygame.K_i or str(getattr(olay, "unicode", "")).lower() in ("i", "ı"):
    ...
    if olay.key == pygame.K_o:
        return _agraphon_test_alchemy_grant()

    if olay.key == pygame.K_p:
        return _agraphon_test_unlock_all_skills()
...
def gelistirici_test_paneli_ciz():
    ...
    rows = (
        ...
        ("CTRL+O", "+1 Aurum Potabile · +1 Quinta Essentia"),
        ("CTRL+P", bt("bütün yetenekleri aç", "unlock all abilities")),
